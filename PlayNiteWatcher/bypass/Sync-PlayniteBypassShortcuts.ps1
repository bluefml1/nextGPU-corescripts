...
. $bootstrapCommon
$bootstrapBypass = Join-Path $script:WatcherRoot "..\BypassShortcutUI.ps1"
if (-not (Test-Path -LiteralPath $bootstrapBypass)) {
    $checkPath = Split-Path -Path $script:WatcherRoot -Parent
    while ($checkPath) {
        $candidate = Join-Path $checkPath "BypassShortcutUI.ps1"
        if (Test-Path -LiteralPath $candidate) { $bootstrapBypass = $candidate; break }
        $checkPath = Split-Path -Path $checkPath -Parent
    }
}
. $bootstrapBypass

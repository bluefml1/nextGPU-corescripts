...
<#
.SYNOPSIS
    Build NextGPU Playnite Steam extensions (SteamLibrary_NextGPU + NextGPUBypassGuard).
.EXAMPLE
...
$guardOut = Join-Path $buildRoot "NextGPUBypassGuard"
$steamTemplate = Join-Path $scriptRoot "extension-templates\SteamLibrary_NextGPU.extension.yaml"
$guardTemplate = Join-Path $scriptRoot "extension-templates\NextGPUBypassGuard.extension.yaml"
...
Write-Host "Building NextGPUBypassGuard..."
Push-Location (Join-Path $scriptRoot "src\NextGPUBypassGuard")
try {
...
if (-not (Test-Path -LiteralPath (Join-Path $guardOut "NextGPUBypassGuard.dll"))) {
    throw "NextGPUBypassGuard.dll not found in $guardOut"
}

Copy-Item -LiteralPath $guardTemplate -Destination (Join-Path $guardOut "extension.yaml") -Force
Write-Host "NextGPUBypassGuard -> $guardOut"

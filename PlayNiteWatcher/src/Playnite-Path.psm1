...
function Resolve-NextGpuSteamExePath {
    <#
    .SYNOPSIS
        Locate steam.exe without hardcoding a data-drive letter (Z:).
    .DESCRIPTION
        Order: HKLM/HKCU InstallPath, then PreferNearPath's drive (e.g. Playnite install),
        then every fixed volume for common Steam folder layouts.
    #>
    [CmdletBinding()]
    param(
    ...
    function Add-SteamExeCandidate {
        param([string]$ExePath)
        if ([string]::IsNullOrWhiteSpace($ExePath)) { return }
        try {
            $full = [System.IO.Path]::GetFullPath($ExePath.Trim().TrimEnd('\'))
        }
        catch {
            $full = $ExePath.Trim()
        }
    ...
    foreach ($keyPath in @(
        ...
        try {
            ...
            if ($installPath) {
                Add-SteamExeCandidate -ExePath (Join-Path $installPath.TrimEnd('\') 'steam.exe')
            }
        }
        catch { }
    }
    ...
    $preferLetter = $null
    if ($PreferNearPath -match '^([A-Za-z]):') {
        $preferLetter = $Matches[1].ToUpperInvariant()
    }

    $letters = New-Object System.Collections.Generic.List[string]
    if ($preferLetter) { [void]$letters.Add($preferLetter) }

    try {
        foreach ($vol in @(Get-Volume -ErrorAction SilentlyContinue | Where-Object {
                    $_.DriveLetter -and $_.DriveType -eq 'Fixed'
                } | Sort-Object DriveLetter)) {
            $L = [string]$vol.DriveLetter
            if ($L) { [void]$letters.Add($L.ToUpperInvariant()) }
        }
    }
    catch {
        try {
            foreach ($drive in @(Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue |
                ...
                [void]$letters.Add($drive.Name.ToUpperInvariant())
            }
        }
        catch { }
    }

    # System drive Program Files (always check even if Get-Volume missed)
    $sys = $env:SystemDrive.TrimEnd('\').TrimEnd(':')
    if ($sys) { [void]$letters.Add($sys.ToUpperInvariant()) }

    $uniqueLetters = @($letters | Select-Object -Unique)
    $relPaths = @(
        'Steam\steam.exe',
        'Games\Steam\steam.exe',
        'SteamLibrary\steam.exe',
    ...
    foreach ($letter in $uniqueLetters) {
        foreach ($rel in $relPaths) {
            Add-SteamExeCandidate -ExePath ('{0}:\{1}' -f $letter, $rel)
        }
    }

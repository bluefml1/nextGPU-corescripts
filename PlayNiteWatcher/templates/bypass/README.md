...
| `Game Shortcuts/*.lnk` | Pre-built RunAsTool shortcuts; **setup copies `{shortcutName}.lnk` and any `launches[].path` leaf ending in `.lnk` from `bypass-sync-list.json`** |
| `Game Shortcuts/*.ps1` / `*.cmd` | Generated at sync when `launches[]` has pre-launch paths |

## Sync list scope

The seed folder may contain many `.lnk` files for reference. **Setup does not bulk-copy the folder.** It copies `{shortcutName}.lnk` plus any `launches[].path` whose leaf ends with `.lnk` for each row in [`config/playnite/bypass-sync-list.json`](../../config/playnite/bypass-sync-list.json). RunAsTool import keeps RNT items whose `FileName` matches those same seed shortcut base names (e.g. pre-launch `Garena Platform.lnk` → import `Garena Platform`).
...
1. Add `launches` for any helper that must start before the shortcut — `.exe` paths or Game Shortcuts `.lnk` paths, e.g. `{ "path": "Z:\\Game Shortcuts\\Garena Platform.lnk", "delaySec": 2 }`.
2. Keep `{shortcutName}.lnk` (and any pre-launch `.lnk` leaves) in the seed Game Shortcuts folder so Setup can copy them.
...
4. Update `bypass-sync-list.json` with the `shortcutName` values (and any pre-launch `.lnk` paths) you want setup to deploy.

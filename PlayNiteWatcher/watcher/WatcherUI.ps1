...
function LoadPlayniteExecutablePath() {
    $bp = Join-Path $scriptPath "..\Playnite-Common.ps1"
    if (-not (Test-Path -LiteralPath $bp)) {
        $cp = Split-Path -Path $scriptPath -Parent
        while ($cp) {
            $c = Join-Path $cp "Playnite-Common.ps1"
            if (Test-Path -LiteralPath $c) { $bp = $c; break }
            $cp = Split-Path -Path $cp -Parent
        }
    }
    . $bp
    try {

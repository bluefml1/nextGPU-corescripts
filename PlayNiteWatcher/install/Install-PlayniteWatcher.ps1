...
function Ensure-PlayniteWatcherAdministrator {
    ...
    if (-not (Get-Command Test-IsAdministrator -ErrorAction SilentlyContinue)) {
        $bypassCommon = Join-Path $script:InstallModuleRoot "..\Playnite-Common.ps1"
        if (-not (Test-Path -LiteralPath $bypassCommon)) {
            $checkPath = Split-Path -Path $script:InstallModuleRoot -Parent
            while ($checkPath) {
                $candidate = Join-Path $checkPath "Playnite-Common.ps1"
                if (Test-Path -LiteralPath $candidate) { $bypassCommon = $candidate; break }

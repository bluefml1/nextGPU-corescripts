...
internal static class DisplayConfigHelper
{
    ...
    private static string GetMonitorDevicePath(ref DISPLAYCONFIG_PATH_INFO path)
    {
    ...
    public static List<DeviceRecord> EnumerateDisplayDeviceIds(bool activeOnly)
    {
        ...
        {
            var devicePath = GetMonitorDevicePath(ref path);
            ...
            {
                var pathBytes = Encoding.Unicode.GetBytes(devicePath);
                deviceId = ToUuidV5Sha1(pathBytes);
            }
...
if ($ListAll) {
    foreach ($d in $devices) {
        $mfg = if ($d.Edid) { [DisplayConfigHelper]::ManufacturerCode([DisplayConfigHelper]::ParseEdidManufacturer($d.Edid)) } else { '?' }
...
if (-not $match) {
    Write-Error "No display path matched (HardwareId='$HardwareId', EDID $ManufacturerId/$ProductCode). Use -ListAll."
    exit 1
}

Write-Output $match.DeviceId

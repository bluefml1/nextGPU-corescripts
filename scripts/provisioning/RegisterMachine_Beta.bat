...
if defined VENDOR_ID if not "!VENDOR_ID!"=="" (
    > "%ProgramData%\nextGPU\vendor-id.txt" echo !VENDOR_ID!
    echo [*] vendor-id.txt saved: !VENDOR_ID!
) else (
    if exist "%ProgramData%\nextGPU\vendor-id.txt" del /f /q "%ProgramData%\nextGPU\vendor-id.txt" >nul 2>&1
    echo [*] vendor-id.txt cleared (no vendor_id)
)
...
:: Let SAM settle so Task Scheduler can resolve COMPUTER\nextGPU SID
echo [*] Waiting for nextGPU account SID to become resolvable...
...
set "ADMINCRED_ACL_PS1=%PROVISIONING_DIR%\Repair-NextGpuAdminCredAcl.ps1"
if exist "%ADMINCRED_ACL_PS1%" (
    echo [*] Refreshing admincred.dat deny ACL for nextGPU ^(separate from user storage^)...
    powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%ADMINCRED_ACL_PS1%" -Quiet
    if !errorlevel! neq 0 (
        echo [!] WARNING: Repair-NextGpuAdminCredAcl.ps1 failed ^(exit !errorlevel!^).
    )
)
echo.

...
set "PS_SCRIPT=%SCRIPT_DIR%\scripts\maintenance\Arrange-GamesApps.ps1"
if not exist "%PS_SCRIPT%" (
    echo ERROR: Arrange-GamesApps.ps1 not found at "%PS_SCRIPT%"
    pause
    exit /b 1
)

echo Running Arrange Games/Apps...
echo.
powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%PS_SCRIPT%"
set "ERR=!errorlevel!"
echo.
if !ERR! neq 0 (
    echo [!] Arrange exited with code !ERR!
) else (
    echo [*] Arrange completed.
)
echo.
pause
exit /b !ERR!

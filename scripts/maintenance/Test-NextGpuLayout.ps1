...
$required = @(
    ...
    @('scripts\runtime\NextGpuOnDemandGpuHostSecrets.ps1', 'On-demand GPU host API key secrets'),
    @('scripts\runtime\Invoke-VendorOfflineShutdown.ps1', 'Vendor offline + shutdown after update'),
    @('scripts\runtime\NextGpuMachineIdentity.ps1', 'Machine identity + status flag helpers'),
    @('scripts\runtime\Invoke-NextGpuMachineIdentity.ps1', 'Bat wrapper for identity/status'),
    @('scripts\runtime\ondemand-gpu-host.env.example', 'On-demand GPU host API key example'),
